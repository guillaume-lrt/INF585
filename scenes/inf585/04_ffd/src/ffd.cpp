...
void ffd_deform(buffer<vec3>& position, grid_3D<vec3> const& grid, buffer<grid_3D<float>>& w_mijk /** You may want to add extra parameters*/)
{
		...
		for (size_t kx = 0; kx < Nx; ++kx) {
			for (size_t ky = 0; ky < Ny; ++ky) {
				for (size_t kz = 0; kz < Nz; ++kz) {
					...
					temp += w_mijk[k](kx, ky, kz) * grid(kx, ky, kz);
				}
			}
		}
		position[k] = temp;
