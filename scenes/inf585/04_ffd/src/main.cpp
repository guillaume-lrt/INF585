...
void compute_weight(buffer<grid_3D<float>>& w_mijk);
...
buffer<grid_3D<float>> w_mijk;	 // precomputation of the Bernstein Polynomial

int const Nx = 4, Ny = 4, Nz = 4;		// shape of the grid
int Mesh_size;
...
int main(int, char* argv[])
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{
			ffd_deform(shape.position, grid, w_mijk); // Call of the deformation - you may need to add extra parameters to this function
...
void create_new_surface() // call this function every time we change of surface
{
	...
	Mesh_size = shape.position.size();
	w_mijk.resize_clear(Mesh_size);
	for (int i = 0; i < Mesh_size; i++) {
		w_mijk[i].resize(Nx, Ny, Nz);
	}
	compute_weight(w_mijk);
	//std::cout << w_mijk[0] << std::endl;
}

float compute_Bernstein_poly(float const u, size_t const i, size_t N) {
		...
		for (size_t kx = 0; kx < Nx; ++kx) {
			for (size_t ky = 0; ky < Ny; ++ky) {
				for (size_t kz = 0; kz < Nz; ++kz) {
					w_mijk[k](kx, ky, kz) = compute_Bernstein_poly(shape.position[k].x, kx, Nx-1)* 
											compute_Bernstein_poly(shape.position[k].y, ky, Ny-1)*
											compute_Bernstein_poly(shape.position[k].z, kz, Nz-1);
				}
			}
		}
